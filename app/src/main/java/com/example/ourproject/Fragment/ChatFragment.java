...
import com.example.ourproject.Model.Chatlist;
...
public class ChatFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        recyclerView=view.findViewById(R.id.chatRecycler_ID);
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));

        firebaseUser= FirebaseAuth.getInstance().getCurrentUser();
        DatabaseReference reference= FirebaseDatabase.getInstance().getReference("message_list").child(firebaseUser.getUid());
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot snapshot:dataSnapshot.getChildren()){
                    ...
                    for (Chatlist chatlist:chatlists){
                        if (profileModel.getId().equals(chatlist.getId())){
                            profileModelList.add(profileModel);
                        }
                    }
                }

                userAdapter=new UserAdapter(getContext(),profileModelList,true);
